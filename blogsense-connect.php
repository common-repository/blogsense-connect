...
*/
//credits also go out to automatic @ http://automattic.com/ for his unfilter wordpress MU code which affects the youtube module in multipress blogs.

add_action('admin_menu', 'wp_blogsense_menu');
add_filter('cron_schedules', 'add_per_min');

define('wp_blogsense_BUTTON_TEXT', 'BlogSense');
define('wp_blogsense_ICON_URL', WP_PLUGIN_URL.'/'.plugin_basename( dirname(__FILE__) ).'/icon.png' );
...
function bs_disable_kses_content() {
remove_filter('content_save_pre', 'wp_filter_post_kses');
}
add_action('init','bs_disable_kses_content',20);
